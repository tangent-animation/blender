...
void LightManager::device_update_points(Device *device, DeviceScene *dscene, Scene *scene)
{
	...
	foreach(Light *light, scene->lights) {
        ...
        float light_buffer = __uint_as_float(light->light_buffer);
		...
		if(light->type == LIGHT_POINT) {
			...
			light_data[light_index*LIGHT_SIZE + 4] = make_float4(max_bounces, light_linking, light_buffer, 0.0f);
		}
	...
	foreach(Light *light, scene->lights) {
		...
		light_data[light_index*LIGHT_SIZE + 4] = make_float4(-1, light_linking, light_buffer, 0.0f);
