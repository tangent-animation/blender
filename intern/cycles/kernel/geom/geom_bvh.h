...
ccl_device_intersect bool scene_intersect(KernelGlobals *kg, const Ray *ray, const uint visibility, Intersection *isect,
					 uint *lcg_state, float difl, float extmax, unsigned int shadow_linking)
{
...
#ifdef __HAIR__
		if(kernel_data.bvh.have_curves)
			return bvh_intersect_volume_hair_motion(kg, ray, isect, shadow_linking);
#endif /* __HAIR__ */

		return bvh_intersect_volume_motion(kg, ray, isect, shadow_linking);
	}
#endif /* __OBJECT_MOTION__ */

#ifdef __HAIR__
	if(kernel_data.bvh.have_curves)
		return bvh_intersect_volume_hair(kg, ray, isect, shadow_linking);
#endif /* __HAIR__ */
...
#ifdef __INSTANCING__
	if(kernel_data.bvh.have_instancing)
		return bvh_intersect_volume_instancing(kg, ray, isect, shadow_linking);
#endif /* __INSTANCING__ */

	return bvh_intersect_volume(kg, ray, isect, shadow_linking);
#else /* __KERNEL_CPU__ */
...
#ifdef __HAIR__
		if(kernel_data.bvh.have_curves)
			return bvh_intersect_volume_all_hair_motion(kg, ray, isect, max_hits, shadow_linking);
#endif /* __HAIR__ */

		return bvh_intersect_volume_all_motion(kg, ray, isect, max_hits, shadow_linking);
	}
#endif /* __OBJECT_MOTION__ */

#ifdef __HAIR__
	if(kernel_data.bvh.have_curves)
		return bvh_intersect_volume_all_hair(kg, ray, isect, max_hits, shadow_linking);
#endif /* __HAIR__ */

#ifdef __INSTANCING__
	if(kernel_data.bvh.have_instancing)
		return bvh_intersect_volume_all_instancing(kg, ray, isect, max_hits, shadow_linking);
#endif /* __INSTANCING__ */

	return bvh_intersect_volume_all(kg, ray, isect, max_hits, shadow_linking);
}
#endif
